...
class QYDateViewController: BaseViewController ,UIPageViewControllerDelegate,UIPageViewControllerDataSource{
    ...
    override func initUI() {
        ...
        titleView.layout.direction = .horizontal
    ...
    func refreshBtnTitle()  {
        btnTitle.setTitle(
            """
            \((pageVC.viewControllers?.first as! QYDateItemViewController).year)年\((pageVC.viewControllers?.first as! QYDateItemViewController).month)月
            """, for: .normal)
    }
}
extension QYDateViewController{
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
  
        let year = self.getBeforeMonth(year: (viewController as! QYDateItemViewController).year, month: (viewController as! QYDateItemViewController).month).year
        let month = self.getBeforeMonth(year: (viewController as! QYDateItemViewController).year, month: (viewController as! QYDateItemViewController).month).month
        return getDateItemVC(year: year, month: month)
    }
    
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        
        let year = self.getAfterMonth(year: (viewController as! QYDateItemViewController).year, month: (viewController as! QYDateItemViewController).month).year
        let month = self.getAfterMonth(year: (viewController as! QYDateItemViewController).year, month: (viewController as! QYDateItemViewController).month).month
        return getDateItemVC(year: year, month: month)
    }
...
//            """, for: .normal)
    ...
    func getDateItemVC(year:Int,month:Int) -> QYDateItemViewController{
        let datevc = QYDateItemViewController.init()
        datevc.year=year
        datevc.month = month
